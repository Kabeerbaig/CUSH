...
static void exePipelines(struct ast_pipeline *pipee)
{
    ...
    else if (strcmp(command->argv[0], "fg") == 0)
    {
            ...
            else
            {
                ...
                if (id == sjob->jid) {
                    if (sjob->status == BACKGROUND) {
                        printf("job is in the background!");
        ...
        else
        {
            ...
            if (jid2job[id] == NULL)
            {
                ...
                if (id == sjob->jid) {
                    ...
                    if (killpg(sjob->pgid, SIGCONT)) {
                        printf("Error sending SIGCONT signal");
                    }
                }
                wait_for_job(sjob);
            ...
            if (command->argv[1] == NULL) {
            printf("job id is missing\n");
        }
        pid_t id = atoi(command->argv[1]);

        if (jid2job[id] == NULL) {
                printf("JOB DOESNT EXIST IN KILL\n");
            } 
            else {
                struct job *sjob = jid2job[id];
                if (id == sjob->jid) {
                    if (killpg(sjob->pgid, SIGKILL)) {
                        printf("Error with killing process");
                    }
            ...
            if (command->argv[1] == NULL) {
                printf("Id not passed in stop!");
            }
            pid_t id = atoi(command->argv[1]);

            if (jid2job[id] == NULL) {
                printf("JOB DOESNT EXIST IN STOP\n");
            }
            ...
            if (id == sjob->jid) {
                ...
                if (killpg(sjob->pgid, SIGSTOP)) {
                    printf("Error with the SIGSTOP singal");
                }
            }
        }
