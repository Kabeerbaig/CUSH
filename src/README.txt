...
The tests are located within the src file with the names:
    history_test.py
    cd_test.py
Which can be ran with the stdriver.py script with the .tst file:
    custom_tests.tst
...
jobs:
    We loop through the job_list struct that acts as a linked list and using the
    print_job() function, we print any jobs that do not have it's status as DONE.
...
bg:
    ...
    Else, we assign the status to the background, send a signal to the process group id to continue,
...
stop:
    ...
    a signal to process group id to STOP the process. We then give the termnial back to the shell
...
Pipes:
    If processes are piped, then we wire the the commands piped to each other so that the 
    standard output will be wired into the next commands standard input. The final process
...
Exclusive Access:
    ...
    later on. We also handle jobs when they are stopped because they need termninal access and 
    give change the status of the job to reflect this. All of this combine ensure that we 
...
cd:
    ...
    directory and if not then we give an error stating that there is no such file or directory.
...
history:
    ...
    associated with it.
    We do this by adding the command line to the history before the it is parsed and executed.
    If it is an event designator, then we take the commandline that the event designator commands
    ...
    the up arrow and down arrow will also work to select previous inputted commands
